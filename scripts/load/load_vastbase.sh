...
EXE_FILE_NAME=${EXE_FILE_NAME:-$(which tsbs_load_vastbase)}
if [[ -z "$EXE_FILE_NAME" ]]; then
    echo "tsbs_load_vastbase not available. It is not specified explicitly and not found in \$PATH"
    exit 1
fi

# Load parameters - common
DATA_FILE_NAME=${DATA_FILE_NAME:-vastbase-data.gz}
DATABASE_USER=${DATABASE_USER:-vbadmin}
DATABASE_NAME=${DATABASE_NAME:-benchmark}
DATABASE_HOST=${DATABASE_HOST:-localhost}
DATABASE_PORT=${DATABASE_PORT:-5432}
DATABASE_PWD=${DATABASE_PWD:-Vbase@admin}

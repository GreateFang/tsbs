...
EXE_FILE_NAME=${EXE_FILE_NAME:-$(which tsbs_run_queries_vastbase)}
if [[ -z "$EXE_FILE_NAME" ]]; then
    echo "tsbs_run_queries_vastbase not available. It is not specified explicitly and not found in \$PATH"
    exit 1
fi
...
for FULL_DATA_FILE_NAME in ${BULK_DATA_DIR}/queries_vastbase*; do

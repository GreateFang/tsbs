...
const pgxDriver = "opengauss" // default driver
const pqDriver = "opengauss"
...
func init() {
	...
	//pflag.String("postgres", "host=vastbase user=vastbase2210 sslmode=disable",
	...
	pflag.String("user", "vastbase2210", "User to connect to PostgreSQL as")
	pflag.String("pass", "bt@1800()", "Password for the user connecting to PostgreSQL (leave blank if not password protected)")
	...
	pflag.Bool("force-text-format", true, "Send/receive data in text format")
	...
	if forceTextFormat {
		...
		panic(fmt.Errorf("vasebase not support driver: %s", pgxDriver))
	}
...
func main() {
	runner.Run(&query.VastbaseDBPool, newProcessor)
}
...
func getConnectString(workerNumber int) string {
	...
	if forceTextFormat {
		connectString = fmt.Sprintf("%s disable_prepared_binary_result=yes ", connectString)
	}

	return connectString
}

func prettyPrintResponse(rows *sql.Rows, q *query.VastbaseDB) {
